...
-- 앱 × 강점태그별 지지 수
-- unnest 결과에는 반드시 `별칭(컬럼명)` 형태로 이름을 준다. `as tag` 만 쓰면
-- 컬럼 이름이 tag 인 익명 테이블이 되어, 다른 표의 tag 와 조인할 때
-- `column reference "tag" is ambiguous` 로 깨진다.
create or replace view public.app_tag_support as
  ...
  cross join lateral unnest(r.chosen_strengths) as d(tag)
  where r.is_hidden = false
  group by r.app_id, d.tag;

-- 앱 × 운영자가 붙여 둔 태그. 배열을 미리 행으로 펼쳐 두면 아래 뷰에서
-- unnest 결과와 다른 표의 tag 컬럼이 이름으로 충돌할 일이 없다.
create or replace view public.app_curated_tags as
  select a.id as app_id, cur.tag
  from public.apps a
  cross join lateral unnest(a.differentiators) as cur(tag);
...
create or replace view public.tag_boost_suggestions as
  ...
  where t.total_reviews >= 10
    ...
    and not exists (
      select 1 from public.dismissed_alerts da
      where da.alert_type = 'boost_suggestion'
        and da.app_id = a.id
        and da.tag = s.tag
...
create or replace view public.tag_accuracy_checks as
  ...
  from public.app_curated_tags c
  join public.apps a              on a.id     = c.app_id
  join public.app_review_totals t on t.app_id = c.app_id
  left join public.app_tag_support s on s.app_id = c.app_id and s.tag = c.tag
  where t.total_reviews >= 10
    and 100.0 * coalesce(s.supporter_count, 0) / t.total_reviews <= 10
    and not exists (
      select 1 from public.dismissed_alerts da
      where da.alert_type = 'accuracy_check'
        and da.app_id = c.app_id
        and da.tag = c.tag
        and (da.expires_at is null or da.expires_at > now())
    );
...
grant select on public.app_curated_tags      to authenticated;
